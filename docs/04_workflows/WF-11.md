...
| Channel Key | Text | Conditional | scope=channel olduqda tələb olunur |
...
| Cloudinary API Key Env Ref | Text | No | CLOUDINARY_API_KEY |
| Cloudinary API Secret Env Ref | Text | No | CLOUDINARY_API_SECRET |
| Generic Upload Endpoint | Text | No | https://my-uploader/upload |
| Generic Auth Header Env Ref | Text | No | STORAGE_AUTH_HEADER |
| S3 Bucket | Text | No | ytb-assets |
| S3 Region | Text | No | eu-central-1 |
| S3 Credential Ref | Text | No | cred_s3 |
| YouTube OAuth Credential Ref | Text | No | cred_youtube_oauth |
| YouTube Channel ID | Text | No | UCxxxx |

## Outputs
- PostgreSQL upsert to `yt_studio_config` table (`key`, `value`, `value_type`, `scope`, `environment`, `channel_key`)
- PostgreSQL upsert to `yt_studio_config` table (`key`, `value`, `value_type`, `scope`, `environment`)
- Success/failure message to user
- Secrets DB-də saxlanmır (API key/secret yalnız env/credentials referansı)

## Nodes plan
1. **Trigger: Form** — n8n FormTrigger node ilə web form render
2. **Node: Parse Form Data** — input normalizasiya + validasiya
3. **Node: Prepare Config Rows** — dot-path `key/value` formatında rows yaratmaq
4. **Node: Save Config to Database** — PostgreSQL upsert (`environment`, `scope`, `channel_key`, `key` conflict target)
...
## Database Schema
```sql
INSERT INTO yt_studio_config (key, value, value_type, scope, environment, channel_key, updated_by, updated_at, is_active)
VALUES ('storage.cloudinary.cloud_name', 'demo', 'string', 'global', 'production', NULL, 'wf-11', NOW(), true)
ON CONFLICT (environment, scope, channel_key, key) DO UPDATE
