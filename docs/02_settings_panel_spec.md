...
## Məqsəd
WF-09 Settings Panel sistemin bütün konfiqurasiyalarını və credentials-larını idarə edir. Bu workflow config DB/Data Table üzərindən işləyir və secrets yalnız credentials kimi saxlanılır.
...
- WF-09 heç bir halda credential dəyərini plain text qaytarmır.
...
- `youtube.privacy_default` (string)
- `youtube.api_credentials` (credential_ref)
- `storage.base_url` (string)
- `storage.signing_key` (credential_ref)
- `tts.provider` (string)
- `tts.api_key` (credential_ref)
- `render.endpoint` (string)
- `render.api_key` (credential_ref)
- `branding.default_tags` (json)
...
- WF-09 audit log-ları ən az 12 ay saxlanılır.
